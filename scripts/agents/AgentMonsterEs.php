...
/**
 * Scraping agent for monster.es.
 */
class AgentMonsterEs extends Agent {
			...
			if ($dom->find('title')) {
				if ($dom->find('div#monsterAppliesContentHolder')){
					...
					foreach($dom->find('link[rel=canonical]') as $id) {
						...
						$str2 = explode('.', $str);
						$job->setForeignId('monster_es_' . trim($str2[0]));
					}
					...
					foreach($dom->find('div#monsterAppliesContentHolder') as $DESCRIPTION) {
						...
						foreach ($DESCRIPTION->find('div#iaactionfixed') as $ret7) {
							$ret7->innertext = '';
							$ret7->outertext = '';
						}
					...
					if (count($dom->find('input#jobLocation')) < 1) {
						$job->addJobLocation("Spain", false, false, 273674);
					} else {
						foreach ($dom->find('input#jobLocation') as $LOCATION) {
							if (strlen(trim($LOCATION->value)) === 0) {
								$job->addJobLocation("Spain", false, false, 273674);
							} else {
								$job->addJobLocation($LOCATION->value, false, false, 273674);
							}						
						}
					}
