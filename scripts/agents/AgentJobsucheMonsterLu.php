...
/**
 * Scraping agent for jobsuche.monster.lu.
 */
class AgentJobsucheMonsterLu extends Agent {
			...
			if ($dom->find('title')) {
				if ($dom->find('div#monsterAppliesContentHolder')){
					...
					foreach($dom->find('comment') as $Link) {
						...
						$str2 = explode('.', $str);
						$job->setForeignId('jobsuche_monster_lu_' . trim($str2[0]));
					}
					...
					foreach($dom->find('div#monsterAppliesContentHolder') as $DESCRIPTION) {
						...
						foreach ($DESCRIPTION->find('div.atbcontent') as $ret8) {
							$ret8->innertext = '';
							$ret8->outertext = '';
						}

						foreach ($DESCRIPTION->find('div.atbheader') as $ret9) {
							$ret9->innertext = '';
							$ret9->outertext = '';
						}

						foreach ($DESCRIPTION->find('div#iaactionfixed') as $ret10) {
							$ret10->innertext = '';
							$ret10->outertext = '';
						}
					...
					if (count($dom->find('input#jobLocation')) < 1) {
						...
						foreach($dom->find('input#jobLocation') as $LOCATION) {	
							if (strlen(trim($LOCATION->value)) === 0) {
								...
								$job->addJobLocation(trim($LOCATION->value), false, false, 273741);
							}						
						}
					}	
