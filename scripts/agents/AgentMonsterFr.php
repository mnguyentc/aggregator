...
/**
 * Scraping agent for monster.fr.
 */
class AgentMonsterFr extends Agent {
			...
			if ($dom->find('title')) {
				if ($dom->find('div#monsterAppliesContentHolder')){
					...
					foreach($dom->find('comment') as $Link) {
						...
						$str2 = explode('.', $str);
						$job->setForeignId('monster_fr_' . trim($str2[0]));
					}
					...
					if (count($dom->find('input#jobLocation')) < 1) {
						...
						foreach ($dom->find('input[id=jobLocation]') as $LOCATION) {
							if (strlen(trim($LOCATION->value)) === 0) {
								...
								$job->addJobLocation($LOCATION->value, false, false, 273682);
							}
						}
					}
