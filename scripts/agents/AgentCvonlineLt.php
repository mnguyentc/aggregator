...
/**
 * Scraping agent for cvonline.lt.
 */
class AgentCvonlineLt extends Agent {
			...
			if ($dom->find('meta[property=og:title]')) {
				if ($dom->find('table#jobad_table')){
					...
					foreach($dom->find('comment') as $Link) {
						...
						$str2 = explode('.', $str);
						$job->setForeignId('cvonline_lt_' . trim($str2[0]));
					}
					...
					if (count($dom->find('span[itemprop=jobLocation]')) < 1) {
						$job->addJobLocation("Lithuania", false, false, 273740);
					} else {
						foreach ($dom->find('span[itemprop=jobLocation]') as $LOCATION) {
							if (strlen(trim($$LOCATION->plaintext)) === 0) {
								$job->addJobLocation("Lithuania", false, false, 273740);
							} else {
								$job->addJobLocation(trim($LOCATION->plaintext), false, false, 273740);	
							}
						}
					}
