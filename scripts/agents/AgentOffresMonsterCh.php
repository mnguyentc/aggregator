...
/**
 * Scraping agent for offres.monster.ch.
 */
class AgentOffresMonsterCh extends Agent {
			...
			if ($dom->find('title')) {
				if ($dom->find('div#monsterAppliesContentHolder')){
					...
					foreach($dom->find('comment') as $Link) {
						...
						$str2 = explode('.', $str);
						$job->setForeignId('offres_monster_ch_' . trim($str2[0]));
					}
					...
					if (count($dom->find('input[id=jobLocation]')) < 1) {
						$job->addJobLocation("Switzerland", false, false, 273650);
					} else {
						foreach ($dom->find('input[id=jobLocation]') as $LOCATION) {
							if (strlen(trim($LOCATION->value)) === 0) {
								$job->addJobLocation("Switzerland", false, false, 273650);
							} else {
								$job->addJobLocation($LOCATION->value, false, false, 273650);
							}
						}
					}
