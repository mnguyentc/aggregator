...
/**
 * Scraping agent for monster.cz.
 */
class AgentMonsterCz extends Agent {
			...
			if ($dom->find('title')) {
				if ($dom->find('div#monsterAppliesContentHolder')){
					...
					foreach($dom->find('link[rel=canonical]') as $Link) {
						$str = substr(strrchr($Link->href, '-'), 1);
						$str2 = explode('.', $str);
						$job->setForeignId('monster_cz_' . trim($str2[0]));
					}
					...
					foreach($dom->find('div#monsterAppliesContentHolder') as $DESCRIPTION) {
						...
						foreach ($DESCRIPTION->find('div#sidecol') as $ret2) {
							$ret2->innertext = '';
							$ret2->outertext = '';
						}
						...
						foreach ($DESCRIPTION->find('div#jobsummary') as $ret4) {
							$ret4->innertext = '';
							$ret4->outertext = '';
						}
					...
					if (count($dom->find('span[itemprop=jobLocation]')) < 1) {
						$job->addJobLocation("Czech Republic", false, false, 273662);
					} else {
						foreach ($dom->find('span[itemprop=jobLocation]') as $LOCATION) {
							if (strlen(trim($LOCATION->plaintext)) === 0) {
								$job->addJobLocation("Czech Republic", false, false, 273662);
							} else {
								$job->addJobLocation(trim($LOCATION->plaintext), false, false, 273662);
							}
						}
					}
